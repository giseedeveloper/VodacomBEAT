...
class SelcomRestClient
{
    ...
    public function post(string $url, array $params, $isOrder = true) : SelcomOrderResponse
    {
        ...
        //Extract response
        $selcomResponse = json_decode($rawResponse, true);
        if ($selcomResponse['resultcode'] != '000') {
            Log::debug('Selcom request failed: ');
            return SelcomOrderResponse::failureWithMessage(false,$rawResponse);
        }
        ...
        $selcomOrderResponse->reference = $selcomResponse['reference'];

        if($isOrder){
            //Extract data elements
            $selcomOrderResponse->gatewayBuyerUuid = $selcomResponse['data'][0]['gateway_buyer_uuid'];
            $selcomOrderResponse->paymentToken = $selcomResponse['data'][0]['payment_token'];
            $selcomOrderResponse->paymentGatewayUrl = $selcomResponse['data'][0]['payment_gateway_url'];
            $selcomOrderResponse->paymentGatewayUrlPlain = base64_decode( $selcomOrderResponse->paymentGatewayUrl);
            $selcomOrderResponse->qr = $selcomResponse['data'][0]['qr'];
        }
